...
if __name__ == "__main__":
    # Analizar el backup más reciente
    backup_dir = "data/backups"
    if os.path.exists(backup_dir):
        archivos = [f for f in os.listdir(backup_dir) if f.endswith('.json')]
        if archivos:
            archivo_reciente = max(archivos, key=lambda x: os.path.getctime(os.path.join(backup_dir, x)))
            print(f"📁 Analizando backup más reciente: {archivo_reciente}")
            analizar_backup(os.path.join(backup_dir, archivo_reciente))
        else:
